...
case class Parameter(
  ...
  def parse(req: CliRequest, args: Seq[String]): Option[(CliRequest, List[String])] =
    kind match {
      case Parameter.SwitchKind => args match {
        case x :: Nil if is_switch(x) => Some((req.add(CliSwitch(x, None)), Nil))
        case x :: xs if is_switch(x) => Some((req.add(CliSwitch(x, None)), xs))
        case _ => None
      }
      case Parameter.PropertyKind => args match {
        case x :: Nil if is_switch(x) => RAISE.invalidArgumentFault(x)
        case x :: xx :: xs if is_switch(x) => Some((req.add(CliProperty(x, CliArgument(xx, None))), xs))
        case _ => None
      }
    }
  ...
  protected def _to_name(p: Any) = p.toString
